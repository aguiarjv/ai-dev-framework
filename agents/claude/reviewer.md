...
`projects/<project-name>/reviews/<plan-id>/` and a reviewer handoff for the
task's `handoffs/` directory or the plan-level `handoffs/` directory, as
applicable. Use clean, actionable-findings, or blocked as the review result. Do
not weaken uncertain evidence into a confirmed finding. The orchestrator
persists both payloads and updates workflow state.
