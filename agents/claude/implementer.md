...
findings and any directly required regression coverage. If it follows an
integration conflict, reconcile only the recorded conflict against the current
plan integration head. If a missing product or architectural decision appears,
stop at a safe checkpoint and return it to the orchestrator instead of guessing.
...
implementation-to-review handoff. Do not mark the task ready-for-integration or
completed; a clean final review makes it ready for integration, and only a
successful merge into the plan branch completes it.
